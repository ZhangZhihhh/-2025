...
#define BIG_STRIDE 0x7FFFFFFF /* max stride, should be positive */
...
stride_pick_next(struct run_queue *rq)
{
     ...
     while (le_next != &(rq->run_list)) {
          struct proc_struct *q = le2proc(le_next, run_link);
          if ((int32_t)(q->lab6_stride - p->lab6_stride) < 0) {
               p = q;
          }
          le_next = list_next(le_next);
     }
#endif
